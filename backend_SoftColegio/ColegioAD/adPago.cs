...
namespace ColegioAD
{
    public class adPago : ad_aglobal
    {
                ...
                using (MySqlCommand cmd = new MySqlCommand("sp_listar_pago", cnMysql))
                {
                    ...
                    using (MySqlDataReader mdrd = cmd.ExecuteReader())
                    {
                        if (mdrd != null)
                        {
                            ...
                            int pos_dtfechafin = mdrd.GetOrdinal("dt_fechafin");
                            ...
                            while (mdrd.Read())
                            {
                                ...
                                enUsuario.SnombrePago = (mdrd.IsDBNull(pos_vnombre) ? "-" : mdrd.GetString(pos_vnombre));
                                ...
                                enUsuario.Sfechafin = (mdrd.IsDBNull(pos_dtfechafin) ? "-" : mdrd.GetString(pos_dtfechafin));
        ...
        public int adRegistrarPago(int adidusuario, int adidnivel, int adidgrado, int adidcurso, string adoperacion
                , int adtipopago, int adtipomoneda, string addescripcion, int addia, int admes, int adanio
                , string adhora, decimal admonto, decimal adigv, string adimg_ruta_1, string adimg_ruta_2
            , string adfecha_ini_pago, string adfecha_fin_pago, string adfecharegistro, string adfechafin)
        {
            try
            {
                int result = -2;
                ...
                cmd.Parameters.Add("_i_dia", MySqlDbType.Int32).Value = addia;
                ...
                cmd.Parameters.Add("_d_igv", MySqlDbType.Decimal).Value = adigv;
                ...
                cmd.Parameters.Add("_v_fecha_fin_pago", MySqlDbType.VarChar, 25).Value = adfecha_fin_pago;
                cmd.Parameters.Add("_b_estado", MySqlDbType.Bit).Value = 1;
                cmd.Parameters.Add("_b_vigente", MySqlDbType.Bit).Value = 1;
        ...
        public int adNotificarPago(int adidusuario, string adfechaacceso, string adfechavalidar)
        {
            try
            {
                int iresultado = 0;
